...
int main() {
  ...
  int area1 = (R1[2]-R1[0]) * (R1[1]-R1[3]); //area of R1
  int area2 = (R2[2]-R2[0]) * (R2[1]-R2[3]); //area of R2

  int combArea = (R2[2] - R1[0]) * (R2[1] - R1[3]); //area of the union of R1 and R2
 
  IoU = (combArea*100)/((area1 + area2) - combArea);  // Remove this line. (It's only provided so that shell code compiles w/out warnings.)
